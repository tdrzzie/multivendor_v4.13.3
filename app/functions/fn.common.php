...
function fn_get_currencies_list($params = array(), $area = AREA, $lang_code = CART_LANGUAGE)
{
    ...
    array_walk($currencies, static function (&$currency) {
        ...
        list($storefronts,) = StorefrontProvider::getRepository()->find(['currency_ids' => $currency['currency_id']]);
...
function fn_get_currency_data($currency_id, $lang_code = DESCR_SL)
{
    ...
    /** @var \Tygh\Storefront\Repository $repository */
    $repository = Tygh::$app['storefront.repository'];
    list($storefronts,) = $repository->find(['currency_ids' => $currency_id]);
    $currency['storefront_ids'] = implode(',', array_keys($storefronts));
...
function fn_is_setup_wizard_panel_available(array $auth)
{
    ...
    return !empty($schema) && !empty($auth['user_type']) && $auth['user_type'] === UserTypes::ADMIN && $auth['is_root'] === YesNo::YES;
}
...
function fn_get_admin_panel_menu_items(array $schema, array $previous_parent = null)
{
    ...
    foreach ($schema as $item_id => $item_data) {
        ...
        foreach ($subitems as $subitem_id => $subitem_data) {
            ...
            $menu_items[(string) $subitem_data['href']] = [
                'id' => $subitem_id,
...
/**
 * Install addon and set it into active status
 *
 * @param string $addon          Internal addon name
 * @param bool   $create_payment True if payment method should be created after install
 *
 * @return bool True if success, false otherwise
 */
function fn_setup_wizard_install_addon($addon, $create_payment = false)
{
    ...
    $processor_data = db_get_row('SELECT processor_id, processor, processor_script FROM ?:payment_processors WHERE addon = ?s', $addon);
    if (empty($processor_data)) {
        return false;
    }

    if (!fn_get_payments(['processor_script' => $processor_data['processor_script']])) {
        fn_update_payment([
...
function fn_setup_wizard_update_money_transfer_type($type)
{
    ...
    foreach ($business_models[$type]['addons'] as $addons) {
        foreach ($addons as $addon_name => $required_status) {
            ...
            if (empty($status) && $required_status === ObjectStatuses::ACTIVE) {
                fn_setup_wizard_install_addon($addon_name, true);
                continue;
            }
 ...
 *
 * Actions performed:
 * - Sets all languages available for a storefront storefront when all languages are assigned to another storefronts.
 *
 * @param \Tygh\Storefront\Storefront  $storefront  Updated storefront
 * @param \Tygh\Common\OperationResult $save_result Storefront update result
 *
 * @return void
 *
 ...
 * @internal
 */
function fn_core_storefront_repository_save_post(Storefront $storefront, OperationResult $save_result)
{
    if (
        $storefront->storefront_id
        || $save_result->isFailure()
    ) {
        return;
    }
...
{
    if ($storefront_id || $old_data['name'] !== 'frontend_default_language') {
        return;
    }
