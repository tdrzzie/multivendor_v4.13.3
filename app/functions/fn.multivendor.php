...
{
    $filters[ProductFilterProductFieldTypes::VENDOR] = array(
        'db_field' => 'company_id',
        'table' => 'products',
        'description' => 'vendor',
        'condition_type' => 'F',
...
function fn_mve_place_order(&$order_id, &$action, &$order_status, &$cart, &$auth)
{
    ...
    if ($order_info['is_parent_order'] != 'Y' && !empty($order_info['company_id'])) {
        ...
        if (fn_are_company_orders_fulfilled_by_marketplace($order_info['company_id'])) {
            $data['marketplace_shipping_amount'] = $order_info['shipping_cost'];
            $order_info['total'] -= $order_info['shipping_cost'];
