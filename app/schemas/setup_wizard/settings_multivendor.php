...
$schema['vendors'] = [
    ...
    'sections'           => [
        [
            'header'   => 'sw.vendor_onboarding',
            'template' => 'views/setup_wizard/components/tabs/sections/vendor_onboarding.tpl',
            'callback' => static function () {
                $setting_data = [];

                $settings = Settings::instance();
                $id = $settings->getId('apply_for_vendor', 'Vendors');
                if ($id) {
                    $setting_data['apply_for_vendor'] = $settings->getData((int) $id);
                }

                $id = $settings->getId('create_vendor_administrator_account', 'Vendors');
                if ($id) {
                    $setting_data['create_vendor_administrator_account'] = $settings->getData((int) $id);
                }

                $id = $settings->getId('vendors_prior_approval', 'vendor_data_premoderation');
                if ($id) {
                    $setting_data['vendors_prior_approval'] = $settings->getData((int) $id);
                }
        ...
        [
            ...
            'setting_data' => Registry::ifGet('addons.vendor_data_premoderation', false),
        ],
        [
