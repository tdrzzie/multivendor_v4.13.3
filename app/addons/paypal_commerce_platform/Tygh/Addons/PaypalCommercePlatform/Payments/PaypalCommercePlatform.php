...
class PaypalCommercePlatform
{
    ...
    protected function buildItems(array $order)
    {
        ...
        if (
            ...
            if (!empty($order['payment_surcharge'])) {
                $items[] = $this->buildPaymentSurcharge($order['payment_surcharge']);
            }
        } else {
            $items[] = [
    ...
    /**
     * Builds payment surcharge item record.
     *
     * @param float $amount Surcharge amount
     *
     * @return array<string, int|float|string> Surcharge item
     *
     * @psalm-return array{
     *   name: string,
     *   unit_amount: array{
     ...
     * }
     */
    protected function buildPaymentSurcharge($amount)
    {
        return [
