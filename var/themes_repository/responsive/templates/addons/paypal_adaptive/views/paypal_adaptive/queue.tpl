    ...
    <form name="payments_form_{$index}" action="{""|fn_url}" method="post" class="payments-form">
        ...
        <div class="ty-step__container{if $pay}-active{/if}">
            ...
            <h3 class="ty-step__title{if $pay}-active{/if}{if $queue.paid && !$pay}-complete{/if} clearfix">
                <span class="ty-step__title-left">{if $queue.paid}<i class="ty-step__title-icon ty-icon-ok"></i>{else}{$index+1}{/if}</span>
...
{capture name="mainbox_title"}<span class="ty-checkout__title">{__("paypal_adaptive_progress_payment_order")}&nbsp;<i class="ty-checkout__title-icon ty-icon-lock"></i></span>{/capture}
