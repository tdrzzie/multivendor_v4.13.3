    ...
    <div class="ty-product-filters__wrapper">
        <div class="ty-product-filters__block">
            <div id="sw_content_{$filter_uid}" class="ty-product-filters__switch cm-combination-filter_{$filter_uid}{if !$collapse} open{/if} cm-save-state {if $filter.display == "Y"}cm-ss-reverse{/if}">
                <span class="ty-product-filters__title">{__("vendor_location.search_vendors_geolocation")}{if $reset_url}<a class="cm-ajax cm-ajax-full-render cm-history" href="{$reset_url|fn_url}" data-ca-event="ce.filtersinit" data-ca-target-id="{$ajax_div_ids}" data-ca-scroll=".ty-mainbox-title"><i class="ty-icon-cancel-circle"></i></a>{/if}</span>
            ...
            <div class="ty-product-filters {if $collapse}hidden{/if}" id="content_{$filter_uid}">
                <div class="ty-product-filters__search ty-filter-products-by-geolocation-filter-address">
                    ...
                    <i class="ty-icon-target cm-filter-geolocation-use-my-location-button ty-vendors-locations-use-my-location" data-ca-filter-geocomplete-elem-id="vendor_search_geolocation_{$block.snapping_id}"></i>
                </div>
        ...
        <div class="ty-product-filters__tools clearfix">
            <a href="{"companies.catalog"|fn_url}" rel="nofollow" class="ty-product-filters__reset-button"><i class="ty-product-filters__reset-icon ty-icon-cw"></i> {__("reset")}</a>
        </div>
    </div>
